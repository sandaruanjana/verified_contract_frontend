...
export function registerRouterNavigationGuards({ router, api }: VueroAppContext) {
  router.beforeEach(async (to, from) => {
    ...
    //       if(user.value.Position === "CONTRACTOR"){
    //         router.push({
    //           name: 'contractor',
    //         })
    //       }else {
    ...
    /*if (Object.keys(user.value).length !== 9) {
      await userSession.logoutUser()
    }*/

    if (
      router.currentRoute.value.path.startsWith('/customer') &&
    ...
    if (
      router.currentRoute.value.path.startsWith('/contractor') &&
      user.value.role !== 'CONTRACTOR'
    ) {
      await userSession.logoutUser()
    }
